...
RegisterNUICallback('fail', function(_, cb)
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            ...
            TriggerEvent('mrf_inventory:client:ItemBox', QBCore.Shared.Items['advancedlockpick'], 'remove')
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent('qb-storerobbery:server:removeLockpick')
            TriggerEvent('mrf_inventory:client:ItemBox', QBCore.Shared.Items['lockpick'], 'remove')
        end
    end
