    ...
    if cachedPoliceAmount[source] >= Config.RequiredCops then
        if plrPed then
            ...
            if dist <= 25.0 then
                ...
                if exports['mrf_inventory']:AddItem(src, Config.VitrineRewards[item]['item'], amount, false, false, 'qb-jewellery:server:vitrineReward') then
                    TriggerClientEvent('mrf_inventory:client:ItemBox', src, QBCore.Shared.Items[Config.VitrineRewards[item]['item']], 'add')
                else
