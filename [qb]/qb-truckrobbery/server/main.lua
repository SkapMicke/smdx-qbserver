...
RegisterServerEvent('AttackTransport:graczZrobilnapad', function()
	...
	exports['mrf_inventory']:AddItem(_source, 'markedbills', bags, false, info, 'AttackTransport:graczZrobilnapad')
	TriggerClientEvent('mrf_inventory:client:ItemBox', _source, QBCore.Shared.Items['markedbills'], 'add')

	local chance = math.random(1, 100)
	TriggerClientEvent('QBCore:Notify', _source, 'You took ' .. bags .. ' bags of cash from the van')

	if chance >= 95 then
		exports['mrf_inventory']:AddItem(_source, 'security_card_01', 1, false, false, 'AttackTransport:graczZrobilnapad')
		TriggerClientEvent('mrf_inventory:client:ItemBox', _source, QBCore.Shared.Items['security_card_01'], 'add')
	end
	Wait(2500)
end)
