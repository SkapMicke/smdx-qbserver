...
$(document).on("keydown", function () {
    ...
    switch (event.keyCode) {
        case 27: // ESC
            var itemoptions = $('.item-options').css("display")
            if (itemoptions == 'block') {
                $('.item-options').css('display', 'none');
            } else {
                Inventory.Close();
            }
            break;
...
$('body').mousemove(function (data) {
    posX = data.clientX
    posY = data.clientY
    var ishereornot = $('.item-options').css("display")
    if (ishereornot == 'none') {
        var widting = $('.item-options').width() / 2
        var hieigh = $('.item-options').height() / 2
        $('.item-options').css('left', (data.clientX - widting) + 68 + 'px').css('top', (data.clientY - hieigh) + 45 + 'px')
    }
});
...
$(document).on("contextmenu", ".item-slot", function (e) {
    ...
    if ($(this).data("item") != null) {
        ...
        dataInventory = $(this).parent().attr("data-inventory");
        if (dataInventory == 'player') {
            $('#use-item').css('display', 'flex');
            $('#give-item').css('display', 'flex');
            var widting = $('.item-options').width() / 2
            var hieigh = $('.item-options').height() / 2
            $('.item-options').css('left', (posX - widting) + 68 + 'px').css('top', (posY - hieigh) + 45 + 'px')
            $('.item-options').css('display', 'block');
        } else {
            $('.item-options').css('display', 'none');
        }
    }
...
$(document).on("click", "#use-item", function (e) {
    let ItemData = dataItem;
    let ItemInventory = dataInventory;
    if (ItemData) {
        if (ItemData) {
            Inventory.Close();
            $.post(`https://${GetParentResourceName()}/UseItem`,
...
$(document).on("click", "#give-item", function (e) {
    let ItemData = dataItem;
    let ItemInventory = dataInventory;
    if (ItemData) {
        ...
        if (ItemData.name !== undefined) {
            if (ItemData.name.split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    $(".inv-options-list").append('<div class="inv-option-item" id="weapon-attachments"><p>ATTACH</p></div>');
                    $("#weapon-attachments").hide().fadeIn(0);
                    ClickedItemData = ItemData;
                } else if (ClickedItemData == ItemData) {
                    $("#weapon-attachments").fadeOut(0, function () {
...
$(document).on("click", ".weapon-attachments-back", function (e) {
    e.preventDefault();
    $("#inventorymain").css({ display: "block" });
...
function FormatAttachmentInfo(data) {
    $.post(`https://${GetParentResourceName()}/GetWeaponData`,
        ...
        function (data) {
            ...
            $(".weapon-attachments-container-title").html(data.WeaponData.label);
            $(".weapon-attachments-container-description").html(data.WeaponData.description);
            $(".weapon-attachments-container-details").html('<span style="font-weight: bold; letter-spacing: .1vh;">Serial:</span><br> ' + ClickedItemData.info.serie + '<br><br><span style="font-weight: bold; letter-spacing: .1vh;">Durability:</span><br> ' + Durability.toFixed() + '% </span> <div class="weapon-attachments-container-detail-durability"><div class="weapon-attachments-container-detail-durability-total"></div></div>');
            $(".weapon-attachments-container-detail-durability-total").css({
                width: Durability + "%",
            });

            var mainWeaponImageSrc = "./attachment_images/" + data.WeaponData.name + ".png";
            if (imageExists(mainWeaponImageSrc)) {
                $(".weapon-attachments-container-image").attr("src", mainWeaponImageSrc);
            } else {
                $(".weapon-attachments-container-image").attr("src", "./attachment_images/noimage.png");
            }
            ...
            if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
                if (data.AttachmentData.length > 0) {
                    ...
                    $.each(data.AttachmentData, function (i, attachment) {
                        var imgSrc = "./images/" + attachment.attachment + ".png";
                        var imgTag = '<img src="' + (imageExists(imgSrc) ? imgSrc : '/attachment_images/noimage.png') + '">';
                        $(".weapon-attachments").append('<div class="item-slot weapon-attachment" id="weapon-attachment-' + i + '"> <div class="item-slot-label"><p>' + attachment.label + '</p></div> <div class="item-slot-img">' + imgTag + '</div> </div>');
                        attachment.id = i;
...
$(document).on("click", "#weapon-attachments", function (e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#inventorymain").css({ display: "none" });
    ...
    if (itemData != null && itemData.info != "") {
        ...
        if (itemData.name == "id_card") {
            ...
            $(".item-info-description").html(
                ...
                "</span></p><p style=\"font-size:11px\"><b>Weight: </b>" + itemData.weight + " | <b>Amount: </b> " + itemData.amount + " | <b>Quality: </b> " + "<a style=\"font-size:11px;color:rgb(0, 255, 191)\">" + Math.floor(itemData.info.quality) + "</a>"
            ...
            if (itemData.info.type == "casing") {
                ...
                $(".item-info-description").html(
                    ...
                    "</span></p><br /><p>" +
                    itemData.description +
                    "</p><p style=\"font-size:11px\"><b>Weight: </b>" + itemData.weight + " | <b>Amount: </b> " + itemData.amount + " | <b>Quality: </b> " + "<a style=\"font-size:11px;color:rgb(0, 255, 191)\">" + Math.floor(itemData.info.quality) + "</a>"
                );
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html("<p>" + itemData.info.costs + "</p><p style=\"font-size:11px\"><b>Weight: </b>" + itemData.weight + " | <b>Amount: </b> " + itemData.amount + " | <b>Quality: </b> " + "<a style=\"font-size:11px;color:rgb(0, 255, 191)\">" + Math.floor(itemData.info.quality) + "</a>");
        } else if (itemData.name == "stickynote") {
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html("<p>" + itemData.info.label + "</p><p style=\"font-size:11px\"><b>Weight: </b>" + itemData.weight + " | <b>Amount: </b> " + itemData.amount + " | <b>Quality: </b> " + "<a style=\"font-size:11px;color:rgb(0, 255, 191)\">" + Math.floor(itemData.info.quality) + "</a>");
            ...
            itemData.name == "pethusky" ||
            itemData.name == "petrottweiler" ||
            itemData.name == "petmtlion" ||
            itemData.name == "petmtlion2" ||
            itemData.name == "petcat" ||
            itemData.name == "petpoodle" ||
            itemData.name == "petpug" ||
            itemData.name == "petretriever" ||
            itemData.name == "petshepherd" ||
            itemData.name == "petwesty" ||
            itemData.name == "petcoyote" ||
            itemData.name == "petrabbit" ||
            itemData.name == "pethen"
        ) {
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html("<p>" + itemData.description + "</p><p style=\"font-size:11px\"><b>Weight: </b>" + itemData.weight + " | <b>Amount: </b> " + itemData.amount + " | <b>Quality: </b> " + "<a style=\"font-size:11px;color:rgb(0, 255, 191)\">" + Math.floor(itemData.info.quality) + "</a>");
        }
    } else {
        $(".item-info-title").html("<p>" + itemData.label + "</p>");
        $(".item-info-description").html("<p>" + itemData.description + "</p><p style=\"font-size:11px\"><b>Weight: </b>" + itemData.weight + " | <b>Amount: </b> " + itemData.amount + " | <b>Quality: </b> " + "<a style=\"font-size:11px;color:rgb(0, 255, 191)\">" + Math.floor(itemData.info.quality) + "</a>");
        ...
        if (
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (
                    ...
                    $.post(`https://${GetParentResourceName()}/getCombineItem`, JSON.stringify({
...
(() => {
    ...
    Inventory.itemBox = function (data) {
        ...
        $itembox.html(
            '<div class="item-k-amount"><p>' + data.itemAmount
            +
...
Sounds.Swap = function () {
    var audio2 = document.getElementById("swap");
    audio2.pause();
...
Sounds.PutBack = function () {
    var audio3 = document.getElementById("putback");
    audio3.pause();
...
Sounds.Take = function () {
    var audio4 = document.getElementById("take");
    audio4.pause();
...
Sounds.Open = function () {
    var audio5 = document.getElementById("open");
    audio5.pause();
...
Sounds.Hover = function () {
    var audio6 = document.getElementById("hover");
    audio6.pause();
